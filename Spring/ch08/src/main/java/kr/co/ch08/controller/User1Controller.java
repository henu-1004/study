...
public class User1Controller {
    ...
    public ResponseEntity<User1DTO> registerJson(@Valid @RequestBody User1DTO user1DTO) {
        log.info("JSON 요청: {}", user1DTO);
        User1DTO savedUser1 = user1Service.save(user1DTO);
        return ResponseEntity.status(HttpStatus.CREATED).body(savedUser1);
    }

    // Multipart 요청 전용
    @PostMapping(value = "/user1/upload", consumes = "multipart/form-data")
    public ResponseEntity<User1DTO> registerMultipart(
            @Valid @ModelAttribute User1DTO user1DTO,
            @RequestPart(required = false) MultipartFile file) {

        log.info("Multipart 요청: {}", user1DTO);
        if (file != null) {
